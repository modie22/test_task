...
const KanbanColumns: React.FC<KanbanColumnsProps> = ({ repoPath }) => {
  ...
  const handleDragStart = (e: React.DragEvent<HTMLDivElement>, issue: Issue, sourceColumn: string) => {
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>, targetColumn: string) => {
    ...
    if (dataString) {
      try {
        ...
        dispatch(moveIssue({ source: data.sourceColumn, destination: targetColumn, issue: data.issue, repoPath }));
      } catch (error) {
