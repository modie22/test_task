...
const issuesSlice = createSlice({
    ...
    reducers: {
      moveIssue: (
        state,
        action: PayloadAction<{ source: string; destination: string; issue: Issue; repoPath: string }>
      ) => {
        const { source, destination, issue, repoPath } = action.payload;
        state.issues[source] = state.issues[source].filter((i) => i.id !== issue.id);
    ...
    builder
      ...
      .addCase(fetchIssues.fulfilled, (state, action) => {
        ...
        state.issues = {
          ...
          "In Progress": action.payload.filter((issue: Issue) => issue.assignee),
          Done: [],
        };
